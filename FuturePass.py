...
class PWModel:
    ...
    def __init__(self, private, proxy, anticapcha_key, SmsHubAPI):
        self.playwright = sync_playwright().start()
        self.SmsHubAPI = SmsHubAPI
    ...
    def Login(self):
        ...
        self.page.goto('https://futurepass.futureverse.app/stuff/', timeout=120000)


        # self.page.wait_for_timeout(1000000)

        pages_len = len(self.context.pages)
        self.page.wait_for_selector('xpath=/html/body/div[2]/div[3]/div/div/button[1]').click()
        ...
        try:
            self.page.wait_for_selector('xpath=//*[@id="__next"]/main/div/div/div/div/div[1]/button[2]', timeout=20000).click()
            self.GetNumber()
            self.page.wait_for_selector('input[aria-describedby=":r0:-helper-text"]').fill('+'+self.number)

            self.page.wait_for_selector('xpath=//*[@id="__next"]/main/div/div/div/div/div[1]/button[2]').click()

            code = self.GetSMS()
            if code == None:
                self.page.wait_for_selector('xpath=//*[@id="__next"]/main/div/div/div/div/div[3]/button[1]').click()
                self.GetNumber()
                self.page.wait_for_selector('input[aria-describedby=":r0:-helper-text"]').fill('+' + self.number)
            ...
            code = list(code)
            self.page.wait_for_selector('input[inputmode="numeric"]')

            numerics = self.page.query_selector_all('input[inputmode="numeric"]')
            ...
            self.page.wait_for_selector('xpath=//*[@id="__next"]/main/div/div/div/div/div[3]/button[2]').click()
        except:
            pass



        self.page.wait_for_selector('label[for="terms"]').click()
        self.page.wait_for_timeout(3000)

        try:
            self.page.wait_for_selector('div.captcha-solver.captcha-solver_inner', timeout=20000).click()
        except:
            self.page.reload()
            self.page.wait_for_selector('label[for="terms"]').click()
            try:
        ...
        try:
            self.page.wait_for_selector('div[data-state="error"]', timeout=10000)
        ...
        self.page.wait_for_selector('div[data-state="solved"]')

        self.page.wait_for_timeout(3000)

        self.page.wait_for_selector('xpath=//*[@id="__next"]/main/div/div/div/div[3]/button').hover()
        self.page.wait_for_selector('xpath=//*[@id="__next"]/main/div/div/div/div[3]/button').click()
        try:
            self.page.query_selector('xpath=//*[@id="__next"]/main/div/div/div/div[3]/button').click()
        except:
            pass


        self.page.wait_for_selector('xpath=//*[@id="navScrollContainer"]/main/div/div[2]/div[1]/div/div/button[1]')
        self.page.wait_for_timeout(5000)


    def GetNumber(self):
        # print(f'https://smshub.org/stubs/handler_api.php?api_key={self.SmsHubAPI}&action=getNumber&service=ot&operator=any&country=16')
        response = requests.get(f'https://smshub.org/stubs/handler_api.php?api_key={self.SmsHubAPI}&action=getNumber&service=ot&operator=any&country=16', proxies=self.proxies)
        # print(response.text)
        if ':' in response.text:
            self.number = response.text.split(':')[-1]
            self.numberID = response.text.split(':')[-2]
        ...
        with open('config', 'r', encoding='utf-8') as file:
            for i in file:
                if '2captcha=' in i.rstrip():
                    anticapcha_key = str(i.rstrip().split('2captcha=')[-1])
                elif 'SmsHubAPI=' in i.rstrip():
                    SmsHubAPI = str(i.rstrip().split('SmsHubAPI=')[-1])
        ...
        while trys < 3:
            try:
                ...
                Model = PWModel(private = privates[count],
                                proxy = proxies[count],
                                anticapcha_key = anticapcha_key,
                                SmsHubAPI = SmsHubAPI)


                Model.TwoCaptcha()
                logger.success(f'{count + 1} | Плагин по решению капчи активирован')
                Model.MMActivation()
                logger.success(f'{count+1} | Метамаск активирован')
                Model.Login()
                logger.success(f'{count+1} | Успешно залогинились на сайт')
                # Model.Task()
